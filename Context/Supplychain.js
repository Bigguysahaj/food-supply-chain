...
//INTERNAL IMPORT
import supplychain from "../Context/Supplychain.json";
...
export const SupplychainProvider = ({ children }) => {
    const DappName = "SuperMarket Dapp";
    ...
    const createShipment = async (items) => {
        ...
        try {
            ...
            createItem = await contract.createShipment(
                receiver,
...
const getAllShipment = async () => {
    try {
        ...
        const allShipmemtns = shipments.map((shipment) => ({
...
const getShipmentsCount = async () => {
    try {
        if (!window.ethereum) return "Install MetaMask to use this app";
...
const getShipment = async (index) => {
    console.log(index*1);
    // multiply to one to convert string to number
    try {
        if(!window.ethereum) return "Install MetaMask to use this app";
